...
# BLOWFISH SMS
#
#   An blowfish encrypted SMS client for Android Phones.
#   
#   August 2010
#   stephen@sa7ori.org
#
...
#
 __author__ = "Stephen A. Ridley <stephen@sa7ori.org>"
import struct, types
import android
import threading
import pprint
...
class SMSThread(threading.Thread):
    """ Just a simple class container for threads."""
    def __init__(self, function_to_run, contact, password, dr):
    ...
    def run(self):
        self.function_to_run(self.contact, self.password, self.dr)

def get_smses(contact_number, password, dr):
    """
        This function polls the unread SMS messages from a specific
        sender to see if they are encrypted, if they are then this
        function displays them and marks the corresponding SMS message
        as read.
    """
    while 1:
        messages_resp = dr.smsGetMessages(True).result
        ids_to_mark_read = []
        if messages_resp != None:
            if len(messages_resp)>0:
                for m in messages_resp:
                    num = m["address"];num = num.strip()
                    num = num.replace("-","")
                    num = num.replace(".","")
                    #print num,":",contact_number
                    if contact_number in num:
                        if "sa7" in m["body"]:
            #            print "%s:%s" % (m["address"],m["body"])
                            dr.dialogCreateAlert(m["address"],recv(m["body"],password))
                            dr.dialogShow()
                            dr.dialogGetResponse()
                            dr.dialogDismiss()
                            ids_to_mark_read.append(int(m["_id"]))
                dr.smsMarkMessageRead(ids_to_mark_read,True)
            else:
    #            continue #change to continue
                pass
        time.sleep(1)

def get_crypto_password(dr):
    """
        This function prompts the user for his crypto password.
        This is a separate function so that it can be easily repeated.
    """
    res = dr.dialogGetInput("Password:","Please enter your encryption password (8 chars or more):","777sa7ori777")
    return res.result

def send(plaintext_sms, contact, password, dr):
    """
        Encrypt and base64 encode outgoing message.
    """
    ...
    print "Outgoing sms: %s" % sms
    dr.smsSend(contact,sms)

def recv(sms,password):
    """
    receive the encrypted sms in the form "sa7cDZ21g=="
    and convert it back to plaintext and return it.
    """
    sms = sms.replace("sa7","")+"==\n" 
    bf = Blowfish(password)
    bf.initCTR()
    pt = bf.decryptCTR(base64.b64decode(sms))
    return pt
 
if __name__ == "__main__":
    dr = android.Android()
    dr.dialogCreateAlert("Information:", "To start you will be asked for the contact that you wish to begin an encrypted SMS conversation with.")
    dr.dialogShow()
    blah = dr.dialogGetResponse()
    dr.dialogDismiss()
    contact = dr.pickPhone().result
    contact = contact.replace("-","")
    contact = contact.replace(".","")
    password = ""
    while ((password == None) or (len(password) < 8)):
        password = get_crypto_password(dr)
    smsthread = SMSThread(get_smses, contact, password, dr)
    dr.dialogCreateAlert("Information:", "Due to encryption and encoding overhead your sms message can only be 90 characters long.")
    dr.dialogShow()
    dr.dialogGetResponse()
    dr.dialogDismiss()
#    get_smses("")
    smsthread.start()
    while 1:
        inp = raw_input(contact+"> ")
        if len(inp) > 90:
            print "Your outgoing sms message is larger than 90 characters."
        if len(inp) > 0:
            send(inp, contact, password, dr)
